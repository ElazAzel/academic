...
- заказчик-наблюдатель видит только разрешенные проекты/потоки; глобальный fallback для этой роли запрещён.
...
## 14a. Дополнительные страницы (compatibility / internal)

Помимо основных сценариев, платформа содержит дополнительные страницы, не попадающие в основной сценарий обучения, но реализованные для операционных нужд:

| Страница | Тип | Роль | Описание |
|---|---|---|---|
| `/chat` | Compatibility | Куратор, Инструктор | Чат с фильтрацией по курсам/потокам, загрузка файлов, автосжатие изображений |
| `/popups` | Internal | Куратор, Админ | Создание и управление всплывающими уведомлениями для студентов |
| `/glossary` | Internal | Куратор, Админ | Глоссарий терминов с категориями |
| `/attendance` | Internal | Инструктор | Посещаемость на основе activity_logs |
| `/offline` | Compatibility | Все | PWA offline fallback |
| `/auth/2fa` | Compatibility | Все | Двухфакторная аутентификация (TOTP) |
| `/docs/[slug]` | Core | Публичные | Статические страницы privacy-policy, terms-of-use, cookie-notice |
| `/api/v1/graphql` | Internal | — | GraphQL endpoint для служебных запросов |
| `/api/v1/seed-temp` | Internal | — | Временный seed endpoint (заблокирован в production через CRON_SECRET) |

Ограничение для `/popups`: администратор может отправлять уведомления по ролям и потокам; куратор может создавать всплывающие уведомления только для закрепленных за ним слушателей и видит только собственные записи. Включение/выключение и удаление попапов — admin-only операции.
