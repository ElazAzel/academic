...
- Scope: Fully configure certificate issuance and verification pipeline. Implement premium dynamic PDF rendering with Cyrillic support, custom QR codes, signatures, seals, and prominent unique certificate numbers, and seed a demo certificate for the course "Стратегическое мышление и управление: Мастер-курс".
- Fixed / Added:
  - **Premium Graphics Assets:** Replaced the corrupted 68-byte placeholder files under `public/assets/certificates/` with high-resolution premium assets (`border.png`, `seal.png`, and `signature.png`) generated using AI image tools.
  - **Cyrillic & Custom Image Support:** Integrated safe inspect-decoding `embedImageSafely` in `server/modules/certificates/service.ts` to seamlessly support both JPG and PNG formats (handling image generator outputs correctly) and dynamically registered custom loaded Cyrillic fonts from `public/assets/fonts/`.
  - **Prominent Unique Certificate Number:** Positioned an elegant centered bold gold-embossed certificate number (`Сертификат № ASA-2026-XXXX`) directly below the course title, alongside a system identifier `Номер: ASA-2026-XXXX` at the bottom left.
  - **Verification Page & UI Integration:** Wired the unique certificate number to show in the online verification page `/certificates/verify/[verificationCode]`, student certificates drawer `/student/certificates`, customer observer certificates `/customer-observer/certificates`, and admin dashboard `/admin`.
  - **Demo Certificate Seeding Script:** Created and executed `scripts/issue-demo-certificate.ts` to provision the "Стратегическое мышление и управление: Мастер-курс", enroll a test student, set completion progress to 100%, and generate a fully verified demo certificate `ASA-2026-50316449`.
- Files added:
  - `scripts/issue-demo-certificate.ts`
  - `public/test-certificate.pdf`
- Files changed:
  - `server/modules/certificates/service.ts`
- Validation:
  - `npx vitest run` - green (all 354 tests passed successfully)
  - `npm run build` - green (Next.js production build succeeded with zero errors across 81 routes)
- Status: green.

## 2026-05-21 - Admin Batch User Importer Implementation & Full Verification

- Author: Antigravity (Principal Engineer & Platform Strategist)
- Scope: Complete all remaining/unfinished features for the LMS platform ("доделай все что не доделал"). Implemented a fully functional interactive CSV batch user importer in the Admin panel to replace the static script-only placeholder tab, resolved all TypeScript type issues, and validated the complete platform.
- Fixed / Added:
  - **Batch User Importer Frontend:** Created a premium interactive drag-and-drop CSV importer `UserBatchImporter` inside `components/admin/user-batch-importer.tsx` supporting:
    - Drag-and-drop file upload with UTF-8 support and raw text input fallback.
    - Active cohorts list dropdown matching specific selectable courses.
    - Interactive tabular preview of parsed users with validation statuses (email regex validation), checkboxes for selection, and role assignment.
    - Secure password generation on the server and dynamic CSV download for temporary user login credentials.
  - **Batch User Importer Backend:** Developed server action `importUsersAction` inside `server/actions/admin.ts` handling user duplicate checks, role assignment mapping, optional automatic cohort enrollment in active cohorts, security logging (`logAudit`), password hashing, and action revalidation.
  - **TypeScript Unification:** Resolved all type safety issues concerning nullable course relations in `CohortSelectOption`, wrapped Lucide components to prevent un-supported HTML attribute props, and normalized button component variants to match LMS style design system.
- Files added:
...
- Validation:
  ...
  - `npx vitest run` - green (354 tests passed)
...
- Author: Antigravity (Principal Engineer & Platform Strategist)
- Scope: Resolve full UI/UX modernization, responsive layout adaptations, native PWA in-app installation prompts (decoupled global triggers), and wire student settings page with Postgres-backed server actions.
- Fixed / Added:
  - **Global PWA Prompts:** Modified `components/lms/pwa-install-prompt.tsx` to hook into browser-native `beforeinstallprompt` events, store them on the global `window.deferredPrompt` context, and emit a custom `pwa-installable` event.
  - **PWA Installer UI:** Created a premium "Установить приложение" sidebar action inside `components/layout/nav-links.tsx` that appears reactively only on installable devices (native prompt on Android/Chrome, Safari step-by-step instructions on iOS) and triggers the installation modal via custom window events.
  - **Student Settings Unlocking:** Replaced the legacy redirect in `app/student/settings/page.tsx` with a premium settings dashboard containing:
    - **Личные данные:** Form fields for Profile, Name, Phone, Organization, Company, and Position.
    - **Настройки уведомлений:** Settings for curator replies, module deadlines, new lessons, assignments graded, email digests, and system messages.
    - **Безопасность:** Instant validated password changes.
  - **Server Action Reuse:** Leveraged unified settings server actions (`@/server/actions/settings`) to securely validate and persist changes in PostgreSQL while triggering real-time audits.
- Files changed:
  - `components/lms/pwa-install-prompt.tsx`
  - `components/layout/nav-links.tsx`
  - `app/student/settings/page.tsx`
- Validation:
  - `npm run lint -- --max-warnings=0` - green
  - `npm run typecheck` - green
  - `npm run test` - green (354 passed / 60 test files)
- Status: green.

## 2026-05-18 - Production demo login repair and M-PR-11 Performance & Scale Pass

- Author: Codex
- Scope: unblock production demo authentication and complete the eleventh implementation package from the 90-day modernization plan.
- Production auth incident:
  - Symptom: demo credentials returned `POST /api/auth/callback/credentials` 401.
  - Root cause: production database schema drift, not bad demo passwords. Auth.js credentials lookup expected `users.totp_secret`, `users.totp_enabled`, and `users.backup_codes`, but the connected database did not have those columns.
  - Fix: added `prisma/migrations/20260518001000_add_user_2fa_columns/migration.sql` and applied it with `npx prisma db execute --file ...`.
  - Added `scripts/check-demo-users.ts` and `npm run users:check-demo` to verify demo users, active status, password hashes, and roles without printing sensitive hashes.
- M-PR-11 performance work:
  - Admin and instructor dashboard/analytics queries now use explicit list limits where screens render summaries.
  - Admin analytics no longer loads every `course_progress` row into the page; course progress/completion summaries use grouped aggregates.
  - Instructor quiz analytics no longer loads all quiz attempts under each quiz; attempt totals, pass counts, and average score use grouped aggregates.
  - Existing M-PR-11 query caps, bounded report exports, scoped curator/super-curator queues, and batched super-curator chat summaries are documented as the current performance baseline.
  - Added targeted indexes for `user_roles.role_id`, `module_progress(module_id,status)`, and `quiz_attempts(quiz_id,passed)` in addition to the existing M-PR-11 indexes.
- Files added:
  - `scripts/check-demo-users.ts`
  - `prisma/migrations/20260518001000_add_user_2fa_columns/migration.sql`
- Files changed:
  - `package.json`
  - `prisma/schema.prisma`
  - `prisma/migrations/20260518000000_performance_scale_pass/migration.sql`
  - `server/actions/dashboard/admin.ts`
  - `server/actions/dashboard/instructor.ts`
  - `app/admin/analytics/page.tsx`
  - `server/auth/options.ts`
  - lint cleanup in `app/layout.tsx`, `app/super-curator/risks/page.tsx`, `components/admin/two-factor-setup.tsx`, `tests/load/smoke-test.js`
  - `docs/full-project-audit.md`, `docs/work-plan.md`, `docs/performance-review.md`, `docs/release-verification.md`
- Production checks:
  - `npm run users:check-demo` - green for all 15 demo accounts; status ACTIVE, password `Password123!`, roles present.
  - Production NextAuth credentials callback returned 200 for `admin@academy.local`, `curator@academy.local`, `supercurator@academy.local`, `student1@academy.local`, and `observer@academy.local`.
  - `npx prisma db execute --file prisma/migrations/20260518000000_performance_scale_pass/migration.sql` - green, indexes applied idempotently.
- Validation:
  - `npm run lint -- --max-warnings=0` - green
  - `npm run typecheck` - green
  - `npm run test` - green, 304 tests / 53 files
  - `npx prisma validate` - green
  - `npm run db:generate` - green
  - `npm run build` - green
- Remaining risk:
  - Connected production DB still lacks reconciled Prisma migration history. Continue using the documented controlled SQL path for urgent schema repairs until `_prisma_migrations` is reconciled in M-PR-12.
- Status: green for demo login repair and M-PR-11 scope.

## 2026-05-18 - M-PR-10 Schema Cleanup Window

- Author: Codex
- Scope: tenth implementation package from the 90-day modernization plan.
- Fixed:
  - `User.status` and `LessonQuestion.status` are confirmed enum-backed in Prisma schema.
  - Existing enum migration `20260514000000_migrate_status_enums` is protected by a dedicated backup-first downtime runbook.
  - Added read-only production preflight to inspect status column types, enum labels, status distributions, and `_prisma_migrations` without mutating data.
  - Runtime question-status queries and mutations now use Prisma `QuestionStatus` constants instead of raw status strings.
- Production finding:
  - Connected database already has `users.status` as `UserAccountStatus` and `lesson_questions.status` as `QuestionStatus`.
  - `_prisma_migrations` is absent, so future migration work must reconcile migration history before running `prisma migrate deploy`.
- Files added:
  - `docs/schema-cleanup-window.md`
  - `scripts/schema-cleanup-preflight.ts`
  - `tests/unit/schema-cleanup-window.test.ts`
- Focused validation:
  - `npx tsx scripts/schema-cleanup-preflight.ts` - green, read-only output shows enum columns and normalized values.
- Full validation:
  - `npm run lint -- --max-warnings=0` - green
  - `npm run typecheck` - green
  - `npm run test` - green, 301 tests / 53 files
  - `npx prisma validate` - green
  - `npm run db:generate` - green
  - `npm run build` - green
  - `npx prisma migrate status` - expected red on connected DB until migration history is reconciled; schema preflight shows enum columns already present.
- Status: green, with operational warning to perform backup and migration-history reconciliation before the production migration window is closed.
