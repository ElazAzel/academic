...
Последние repo-local gates по `docs/updates.md`: `npm run verify` проходит, включая banned-patterns, lint, typecheck, 508/508 Vitest tests и production build. Это подтверждает repo-local code health, но не закрывает release-ready статус.
...
| Access/privacy negative paths | `partial` | unit/integration/e2e coverage для ownership и role scope; signed lesson media URL, reports API/preview/async job/status, certificate designer preview, certificate PDF observer scope, report job status owner/safe-download URL, and chat upload negative tests; course-builder inline mutations require course write; popup management scoped by creator for curators, admin-only diagnostics/toggle/delete/cohort selector, and course-owned cohort deadline reads |
