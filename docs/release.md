...
Локальные gates для этой итерации: Prisma boundary scan, E2E `networkidle` scan и `npm run typecheck`. Полный `npm run verify` и release/staging `npm run verify:release` остаются обязательными перед выпуском. Внешние задачи DPA, ротации скомпрометированного Supabase-пароля и git purge `__dbcheck.mjs` остаются операционными runbook-задачами, а не repo-local кодовыми блокерами.
...
| Access/privacy negative paths | `partial` | unit/integration/e2e coverage для ownership и role scope; added signed lesson media URL negative tests for no enrollment, sequential lock, and guessed foreign media ID |
